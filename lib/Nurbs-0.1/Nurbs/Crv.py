...
class Crv:
 ...
 NOTES:
    ...
    def __init__(self, cntrl, uknots):
        ...
        self.uknots = uknots
        cntrl = Numeric.asarray(cntrl, Numeric.Float)
        (dim, nu) = cntrl.shape
        if dim < 2 or dim > 4:
            raise NURBSError, 'Illegal control point format'
        elif dim < 4:
            self.cntrl = Numeric.zeros((4, nu), Numeric.Float)
            self.cntrl[0:dim,:] = cntrl
            self.cntrl[-1,:] = Numeric.ones((nu,))
        else:
            self.cntrl = cntrl
        # Spline degree
    ...
    def trans(self, mat):
        ...
        self.cntrl = Numeric.dot(mat, self.cntrl)
    ...
    def bounds(self):
        ...
        ww = Numeric.resize(self.cntrl[-1,:], (3, self.cntrl.shape[1]))
        cntrl = Numeric.sort(self.cntrl[0:3,:]/ww)
        return Numeric.asarray([cntrl[0,0], cntrl[1,0], cntrl[2,0],
                                cntrl[0,-1], cntrl[1,-1], cntrl[2,-1]], Numeric.Float)
                                
    def pnt3D(self, ut):
        "Evaluate parametric point[s] and return 3D cartesian coordinate[s]"
        val = self.pnt4D(ut)
        return val[0:3,:]/Numeric.resize(val[-1,:], (3, val.shape[1]))
	...
	Depends on the dislin plotting library."""
        ...
        if minx == maxx:
            minx -= 1.
            maxx += 1.
        maxminy = Numeric.sort(self.cntrl[1,:]/self.cntrl[3,:])
        miny = maxminy[0]
        maxy = maxminy[-1]
        if miny == maxy:
            miny -= 1.
            maxy += 1.
        maxminz = Numeric.sort(self.cntrl[2,:]/self.cntrl[3,:])
        minz = maxminz[0]
        maxz = maxminz[-1]
        if minz == maxz:
            minz -= 1.
...
class Line(Crv):
    ...
    def __init__(self, p1 = (0,0,0), p2 = (1,0,0)):
        Crv.__init__(self, Numeric.transpose([p1,p2]), [0,0,1,1])
                       
class Polyline(Crv):
    """A polyline
	Example: c = Polyline([[0,0],[5,2],[10,8]])"""
    def __init__(self, pnts):
        pnts = Numeric.transpose(Numeric.asarray(pnts, Numeric.Float))
        npnts = pnts.shape[1]
        if npnts < 3:
            raise NURBSError, 'Point sequence error'
        cntrl = Numeric.zeros((pnts.shape[0], 2 * npnts - 2), Numeric.Float)
        cntrl[:,0] = pnts[:,0]
        cntrl[:,-1] = pnts[:,-1]
        cntrl[:,1:-2:2] = pnts[:,1:-1]
        cntrl[:,2:-1:2] = pnts[:,1:-1]
        uknots = Numeric.zeros(npnts * 2, Numeric.Float)
        uknots[0::2] = Numeric.arange(npnts)
        uknots[1::2] = Numeric.arange(npnts)
...
class Arc(Crv):
    ...
    def __init__(self, radius = 1.,center = None, sang = 0., eang = 2*math.pi):
        ...
        coefs = Numeric.zeros((4, 2*narcs+1), Numeric.Float)   # nurb control points of arc
